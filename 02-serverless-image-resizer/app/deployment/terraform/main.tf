locals {
  ...
  styled_bucket               = "localstack-thumbnails-app-styled"
...
resource "aws_s3_bucket" "styled_bucket" {
  bucket = local.styled_bucket
}
...
resource "aws_ssm_parameter" "styled_bucket_ssm" {
  name  = "/localstack-thumbnail-app/buckets/styled"
  type  = "String"
  value = aws_s3_bucket.styled_bucket.bucket
}
...
resource "aws_iam_policy" "list_lambda_s3_buckets" {
  ...
  policy = templatefile("policies/list_lambda_s3_buckets.json.tpl", {
    ...
    styled_bucket         = aws_s3_bucket.styled_bucket.bucket
  })
}
...
# Style Transfer lambda

resource "aws_iam_role" "styletransfer_lambda_role" {
  name               = "StyleTransferLambdaRole"
  assume_role_policy = file("policies/lambda.json")
}

resource "aws_iam_policy" "styletransfer_lambda_s3_buckets" {
  name = "StyleTransferLambdaS3Buckets"
  policy = templatefile("policies/styletransfer_lambda_s3_buckets.json.tpl", {
    images_bucket = aws_s3_bucket.images_bucket.bucket,
    styled_bucket = aws_s3_bucket.styled_bucket.bucket
  })
}

resource "aws_iam_role_policy_attachment" "styletransfer_lambda_s3_buckets" {
  role       = aws_iam_role.styletransfer_lambda_role.name
  policy_arn = aws_iam_policy.styletransfer_lambda_s3_buckets.arn
}

resource "aws_iam_role_policy_attachment" "styletransfer_lambda_ssm" {
  role       = aws_iam_role.styletransfer_lambda_role.name
  policy_arn = aws_iam_policy.lambdas_ssm.arn
}

resource "aws_lambda_function" "styletransfer_lambda" {
  function_name = "styletransfer"
  filename      = "${local.root_dir}/lambdas/styletransfer/lambda.zip"
  handler       = "handler.handler"
  runtime       = "python3.11"
  timeout       = 60
  role          = aws_iam_role.styletransfer_lambda_role.arn
  source_code_hash = filebase64sha256("${local.root_dir}/lambdas/styletransfer/lambda.zip")

  environment {
    variables = merge(local.env_variables, {
      GEMINI_API_KEY = var.gemini_api_key
    })
  }
}

resource "aws_lambda_function_url" "styletransfer_lambda_function" {
  function_name      = aws_lambda_function.styletransfer_lambda.function_name
  authorization_type = "NONE"
}
...
output "styletransfer_lambda_function_url" {
  value = aws_lambda_function_url.styletransfer_lambda_function.function_url
}
