...
def get_bucket_name_styled() -> str:
    parameter = ssm.get_parameter(Name="/localstack-thumbnail-app/buckets/styled")
    return parameter["Parameter"]["Value"]
...
def handler(event, context):
    ...
    # collect the associated styled images
    styled_bucket = get_bucket_name_styled()
    images = s3.list_objects(Bucket=styled_bucket)
    for obj in images.get("Contents", []):
        if obj["Key"] not in result:
            continue
        result[obj["Key"]]["Styled"] = {
            "Size": obj["Size"],
            "URL": s3.generate_presigned_url(
                ClientMethod="get_object",
                Params={"Bucket": styled_bucket, "Key": obj["Key"]},
                ExpiresIn=3600,
            ),
        }
